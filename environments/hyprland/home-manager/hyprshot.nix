...
in
{
  ...
  wayland.windowManager.hyprland.settings.bind = [
    ", Print, exec, ${pkgs.writeShellScript "hyprshot-region-edit" ''
      ${hyprshotBin} --freeze --mode region --raw --silent | ${sattyBin} --filename - --output-filename ${screenshotDir}/screenshot-$(date +%Y-%m-%d_%H-%M-%S).png --copy-command ${pkgs.wl-clipboard}/bin/wl-copy
    ''}"
    ...
    "SUPER, Print, exec, systemd-run --user ${pkgs.writeShellScript "hyprshot-window-edit" ''
      ${hyprshotBin} --freeze --mode window --raw --silent | ${sattyBin} --filename - --output-filename ${screenshotDir}/screenshot-$(date +%Y-%m-%d_%H-%M-%S).png --copy-command ${pkgs.wl-clipboard}/bin/wl-copy
    ''}"
