...
let
  ...
  name = "${namespace}-faraday";
  inherit (config.virtualisation.oci-containers.containers."${name}") ip httpPort;
  image = "ghcr.io/grand-board/faraday-cage/server:latest";
  domain = "faraday.${namespace}.web.id";
  tinyauth = {
...
in
{
  ...
  sops.secrets."github/grandboard/faraday.env" = {
    sopsFile = ../../../secrets/grandboard/faraday.env;
    format = "dotenv";
    key = "";
  };
  virtualisation.oci-containers.containers."${name}" = {
    ...
    environmentFiles = [
      config.sops.secrets."github/grandboard/faraday.env".path
    ];
  };
  ...
  systemd.services."podman-${name}-update" = {
    description = "update faraday container";
    script = ''
      set -e
  ...
  services.webhook.hooks.deploy-faraday = {
    ...
    response-message = "faraday deployment triggered";
  };
}
