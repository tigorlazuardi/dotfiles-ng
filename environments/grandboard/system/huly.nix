...
let
  ...
  name = "${namespace}-huly";
  domain = "huly.${namespace}.web.id";
  dataDir = "/var/lib/${namespace}/huly";
  inherit (config.users.users.${namespace}) uid;
  inherit (config.users.groups.${namespace}) gid;
...
{
  sops = {
    secrets."${namespace}/huly.env" = {
      sopsFile = ../../../secrets/${namespace}/huly.env;
      format = "dotenv";
      key = "";
    };
    templates."${namespace}/huly.oidc.env".content = ''
      OPENID_CLIENT_ID=${config.sops.placeholder."${namespace}/dex/clients/huly/client_id"}
      OPENID_CLIENT_SECRET=${config.sops.placeholder."${namespace}/dex/clients/huly/client_secret"}
    '';
  };
  virtualisation.oci-containers.containers =
    let
      baseEnv = rec {
        ...
        OPENID_ISSUER = "https://auth.${namespace}.web.id";
      };
      environmentFiles = [
        config.sops.templates."${namespace}/huly.oidc.env".path
        config.sops.secrets."${namespace}/huly.env".path
      ];
    in
    {
      ...
      "podman-${name}-elastic" = {
        ...
        postStart = ''
          ...
          for i in `seq $attempts`; do
            if ${pkgs.netcat}/bin/nc -z 10.88.21.3 9200 > /dev/null; then
              exit 0
            fi
