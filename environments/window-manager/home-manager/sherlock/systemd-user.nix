...
let
  heartIcon =
    pkgs.writeText "heart.svg" # svg
      ''
        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#ffffff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-heart-pulse-icon lucide-heart-pulse"><path d="M2 9.5a5.5 5.5 0 0 1 9.591-3.676.56.56 0 0 0 .818 0A5.49 5.49 0 0 1 22 9.5c0 2.29-1.5 4-3 5.5l-5.492 5.313a2 2 0 0 1-3 .019L5 15c-1.5-1.5-3-3.2-3-5.5"/><path d="M3.22 13H9.5l.5-1 2 4.5 2-7 1.5 3.5h5.27"/></svg>
      '';
  documentIcon =
    pkgs.writeText "document.svg" # svg
      ''
        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#ffffff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-file-code-icon lucide-file-code"><path d="M10 12.5 8 15l2 2.5"/><path d="m14 12.5 2 2.5-2 2.5"/><path d="M14 2v4a2 2 0 0 0 2 2h4"/><path d="M15 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7z"/></svg>
      '';
  serviceIcon =
...
in
{
  ...
  home.packages = [
    (pkgs.writers.writeJSBin "sherlock-systemd-user" { } ''
      ...
      const actions = [
        {
          title: `Show ''${systemdUnit} Status`,
          description: `''${entry.load} ''${entry.active} ''${entry.sub} · Show ''${systemdUnit} status`,
          field: "exec",
          icon: "${heartIcon}",
          hidden: {
            exec: `systemd-run --user ${config.programs.sherlock.terminal} bash -c "systemctl --user status ''${systemdUnit} | nvim -R -M"`,
          },
        },
        {
